...
public class EmployeeSpecification implements Specification<Employee> {
	...
	public Predicate toPredicate(Root<Employee> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		case DOES_NOT_CONTAIN:
			if (searchCriteria.getFilterKey().equals("deptName")) {
				return cb.notLike(cb.lower(departmentJoin(root).<String>get(searchCriteria.getFilterKey())),
						"%" + strToSearch + "%");
			}
		...
		case EQUAL:
			if (searchCriteria.getFilterKey().equals("deptName")) {
				...
				return cb.equal(departmentJoin(root).<String>get(searchCriteria.getFilterKey()),
		...
		case NOT_EQUAL:
			if (searchCriteria.getFilterKey().equals("deptName")) {
				return cb.notEqual(departmentJoin(root).<String>get(searchCriteria.getFilterKey()),
						searchCriteria.getValue());
			}
